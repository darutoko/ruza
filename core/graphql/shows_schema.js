	...
	query: {
		show: {
			...
			resolve(source, arguments, context, info) {
				return db.show.one({ id: arguments.id }).then(res => {
		...
		seasons: {
			...
			resolve(source, arguments, context, info) {
				return db.season.list({ show_id: arguments.show_id }).then(res => res.rows)
			},
		},
	},
	...
	mutation: {
		addShow: {
			...
			async resolve(source, arguments, context, info) {
				if (!context.user.isAdmin) throw new Error("Access denied")
...
RETURNING *;`,
			...
			async resolve(source, arguments, context, info) {
				...
				try {
					let result,
						show,
						values = [],
						keys = Object.keys(arguments).filter(key => ["current_season", "search", "uploaded"].includes(key))

					if (keys.length) {
						values = keys.map(key => arguments[key])
						result = await db.query(
							`UPDATE public.show SET ${keys.map((key, i) => key + " = $" + (i + 1)).join(", ")} WHERE id = \$${
								keys.length + 1
							} RETURNING *;`,
							[...values, arguments.id]
						)
					...
					if (arguments.directory) {
						...
						result = await db.query(`UPDATE public.season SET directory = $1 WHERE show_id = $2 AND num = $3;`, [
							arguments.directory,
							show.id,
							show.current_season,
						])
		...
		deleteShow: {
			...
			async resolve(source, arguments, context, info) {
				...
				try {
					let result = await db.query(`DELETE FROM public.show WHERE id = $1 RETURNING *;`, [arguments.id])
